...
router.get('/', async (req, res) => {
    try {
        ...
        if (!config.authentication?.remember_pin) {
...
router.get('/', async (req, res) => {
    try {
        ...
        const backups = files
            ...
            .map(f => ({
                filename: f,
                created: f.replace(/[^0-9-]/g, '').replace(/^-|-$/g, ''),
                downloadUrl: `/api/backups/${f}`
            }))
...
router.get('/:filename', async (req, res) => {
    try {
        const backupFile = path.join(process.env.DATA_DIR || './data', 'backups', req.params.filename);
